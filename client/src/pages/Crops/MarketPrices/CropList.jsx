...
export default function CropList() {
  ...
  useEffect(() => {
    ...
    async function loadApmc() {
      try {
        const apmcs = await getApmcMarkets({ lat, lng });

        if (cancelled) return;

        setApmc(
          apmcs.find((a) => a.id === apmcId) ?? null
        );
      } catch {
        if (cancelled) return;

        // Treat a failed APMC lookup as unavailable.
        setApmc(null);
      }
    }
    ...
    async function loadCropPrices() {
      try {
        const prices = await getCropPricesForApmc(apmcId);
        ...
        // Keep the page usable if crop-price data fails.
        setRows([]);
      }
    }
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      row.crop.name.toLowerCase().includes(q)
    );
  }, [rows, query]);
            ...
            <tbody>
              {filteredRows.map((row) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(
                      `/market-prices/${apmcId}/${row.cropId}`
                    )
                  }
                >
                  <td>
                    <span className="mp-crop-icon-sm">
                      {row.crop.icon}
                    </span>
                    {row.crop.name}
                  </td>
